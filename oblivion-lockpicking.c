...
#define FONT_SIZE_START_MENU_TITLE 60
#define FONT_SIZE_START_MENU_SUBTITLE1 40
...
int main(void)
{
    ...
    // Initialize variables for start menu
    GameState gameState = GameStateStartMenu;
    const char *textStartMenuTitle = "Oblivion Lockpicking Game";
    int widthStartMenuTitle = MeasureText(textStartMenuTitle, FONT_SIZE_START_MENU_TITLE);
    const char *textStartMenuSubtitle1 = "Click anywhere on the screen to start playing";
    int widthStartMenuSubtitle1 = MeasureText(textStartMenuSubtitle1, FONT_SIZE_START_MENU_SUBTITLE1);
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        if (gameState == GameStateStartMenu)
        {
            ...
            Vector2 posStartMenuTitle;
            posStartMenuTitle.x = screenWidth / 2 - widthStartMenuTitle / 2;
            posStartMenuTitle.y = screenHeight / 2 - FONT_SIZE_START_MENU_TITLE / 2;
            Vector2 posStartMenuSubtitle1;
            posStartMenuSubtitle1.x = screenWidth / 2 - widthStartMenuSubtitle1 / 2;
            posStartMenuSubtitle1.y = screenHeight / 2 - FONT_SIZE_START_MENU_SUBTITLE1 / 2 + 1.5*FONT_SIZE_START_MENU_SUBTITLE1;
            BeginDrawing();
            ClearBackground(BROWN);
            DrawText(textStartMenuTitle, posStartMenuTitle.x, posStartMenuTitle.y, FONT_SIZE_START_MENU_TITLE, WHITE);
            DrawText(textStartMenuSubtitle1, posStartMenuSubtitle1.x, posStartMenuSubtitle1.y, FONT_SIZE_START_MENU_SUBTITLE1, WHITE);
            EndDrawing();
        }
        else if (gameState == GameStatePlaying)
        {
            ...
            if (IsKeyPressed(KEY_LEFT) || IsKeyPressed(KEY_RIGHT)) // Move the lockpick left or right
            {
                ...
                if (lockpickPinPosition < 0)
                    lockpickPinPosition = 0;
                if (lockpickPinPosition >= maxPins)
                    lockpickPinPosition = maxPins - 1;
                if (oldPosition != lockpickPinPosition)
                    lockpickMotionState = IsKeyPressed(KEY_LEFT) ? 1 : 2;
